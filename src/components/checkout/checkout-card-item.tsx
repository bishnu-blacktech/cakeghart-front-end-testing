...
export const CheckoutItem: React.FC<{ item: Item }> = ({ item }) => {
  ...
  return (
    <div
      className={`group w-11/12 h-auto flex justify-start items-center bg-skin-fill py-4 md:py-7 border-b border-skin-one border-opacity-70 relative last:border-b-0`}
    >
      ...
      <div className="flex w-full overflow-hidden items-start justify-between">
        <div className="ps-3 md:ps-4">
          ...
          <Counter
            ...
            onIncrement={() =>
              addItemToCart(item, item.quantity ? 1 + item.quantity : 1)
            }
            onDecrement={() => {
