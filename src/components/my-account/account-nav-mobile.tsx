    ...
    <div>
      {options?.map((data, index) => {
        return (
          <div>
            <button
              className="mt-2 w-full flex items-center text-sm lg:text-15px text-skin-base py-3 px-4 md:px-5 cursor-pointer focus:outline-non bg-gray-100 rounded"
              onClick={() => handleItemClick(data.slug)}
            >
              <span className="block  w-full text-center">{t(data?.name)}</span>
            </button>
          </div>
        );
      })}
      ...
      <button
        className="mt-2 w-full flex items-center text-sm lg:text-15px text-skin-base py-3 px-4 md:px-5 cursor-pointer focus:outline-non bg-gray-100"
        onClick={() => logout()}
      >
        {/* <span className="flex-shrink-0 flex justify-center">
          <LogoutIcon className="w-5 md:w-[22px] h-5 md:h-[22px]" />
        </span> */}
        <span className="block  w-full text-center">{t('text-logout')}</span>
      </button>
    </div>
  );
